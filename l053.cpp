    ...
    for(int y = 0; y < height; y++){
        for(int x = 0; x < width; x++){
            if((x == 0) || (x == (width-1)) || (y == 0) || (y == (height-1))){
                ...
                gradient[x][y] = grad;
                if(grad > THRESHOLD_1){//THRESHOLD_1 is defined at the top of the file (should be around 80-100)
                    outputVector[x][y] = 0;
                }
                else{
                    outputVector[x][y] = 1;
    ...
    for(int y = 0; y < height; y++){
        for(int x = 0; x < width; x++){
            if((x == 0) || (x == (width-1)) || (y == 0) || (y == (height-1))){
                ...
                //gradient[x][y] = grad;
                if(grad < THRESHOLD_1){//Anything between t1 and t2 is a weak edge
                    gradient[x][y] = 0;
                }
                else if(grad < THRESHOLD_2){//anything above t2 is a strong edge
                    gradient[x][y] = 1;
                }
                else{
                    gradient[x][y] = 2; //anything below t1 is not an edge.
                }
            }
        }
    }//gradient contains the image with the sobal operator in place.
    //apply areaFill to the vector2 gradient...
    areaFill(gradient, width, height);
